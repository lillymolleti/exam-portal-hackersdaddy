...
import { LogOut, Home, FileText, BarChart, Clock, X } from "lucide-react";
  ...
  const navLinks =
    role === "admin"
      ? [
          ...
          { name: "Active Students", path: `/${role}/students`, icon: Home },
          { name: "Questions", path: `/${role}/questions`, icon: Home },
      ...
      : [
          ...
          { name: "Results", path: `/student/results`, icon: BarChart },
        ...
        <nav className="flex-1 space-y-2">
          {navLinks.map((link) => (
            <NavLink
              ...
              className={({ isActive }) => {
                console.log(
                  `Sidebar: NavLink rendered for ${link.name}, isActive:`,
                  isActive
                );
